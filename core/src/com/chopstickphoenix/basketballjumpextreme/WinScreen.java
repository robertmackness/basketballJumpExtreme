package com.chopstickphoenix.basketballjumpextreme;

import com.badlogic.gdx.Gdx;
import com.badlogic.gdx.ScreenAdapter;
import com.badlogic.gdx.graphics.GL20;
import com.badlogic.gdx.graphics.OrthographicCamera;
import com.badlogic.gdx.graphics.g2d.TextureRegion;
import com.badlogic.gdx.graphics.g2d.BitmapFont.HAlignment;

public class WinScreen extends ScreenAdapter {
	basketballJumpExtreme game;
	OrthographicCamera cam;
	TextureRegion agent;
	String[] messages = { "Agent: What are you \ndoing here?",
						  "Maj. Chaos: \nI'm here for \nthe show!",
						  "Agent: Wrong arena \nbuddy!",
						  "Maj. Chaos: \nThis always \nhappens!!!",
						  "Agent: Better get \nmoving",
						  "Maj. Chaos: \nOH \nYEAAAAHHH",
						  "Agent: \nWhat a \nmoron"
			};
	int currentMessage = 0;
	
	public WinScreen(basketballJumpExtreme game) {
		this.game = game;
		cam = new OrthographicCamera();
		cam.setToOrtho(false, 320, 480);
		agent = new TextureRegion(Assets.arrow.getTexture(), 210, 122, -40, 38);
	}
	
	@Override
	public void render(float delta) {
		if(Gdx.input.justTouched()) {
			currentMessage++;
			if(currentMessage == messages.length) {
				currentMessage--;
				game.setScreen(new MainMenuScreen(game));
			}
		}
		
		Gdx.gl.glClear(GL20.GL_COLOR_BUFFER_BIT);
		cam.update();
		game.batcher.setProjectionMatrix(cam.combined);
		game.batcher.begin();
		game.batcher.draw(Assets.backgroundRegion, 0, 0);
		game.batcher.draw(Assets.castle, 60, 120, 200, 200);
		game.batcher.draw(Assets.wrestlerFall.getKeyFrame(0, Animation.ANIMATION_LOOPING), 120, 200);
		Assets.font.drawMultiLine(game.batcher, messages[currentMessage], 0, 400, 320, HAlignment.CENTER);
		game.batcher.draw(agent,150, 200);
		game.batcher.end();
	}
}
